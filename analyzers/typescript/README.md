...
```sh
npm run extract -- --source fixtures/typescript/orders/baseline/src --service orders --revision aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
```
...
```sh
npm run extract -- --source fixtures/typescript/orders/baseline/src --service orders --revision aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa > "$(mktemp -t api-truth-orders)"
```
...
labels, unsupported IR versions, out-of-root inputs, source-digest mismatches,
and resource-limit failures without printing source-sensitive details.
...
|---|---|---|
| Express setup | Default-import `express()` apps; named/aliased `Router()` and default-import `.Router()` receivers | Unknown receivers and route-builder chains |
...
| Responses | Explicit 100–599 status calls, explicit media types, JSON/send/end serialization shapes, and middleware error responses | Missing/dynamic/invalid statuses, missing media types, and unresolved values or spreads |
